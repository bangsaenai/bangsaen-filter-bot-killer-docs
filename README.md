...
### Does the 1,000,000 requests/month quota include blocked bad bots?
...
Whether the request receives an `HTTP 200 OK` or an `HTTP 403 Forbidden`, computational work was performed at the Edge to shield your origin.

---

### 💡 Why $19/mo for 1M Requests is Still Insane ROI

Consider a real-world scenario during a ticket sale or product drop:

* **Total Monthly Traffic:** 1,000,000 Requests
* **Attack Volume:** 800,000 requests generated by `sqlmap`, scrapers, or checkout bots.
* **Legitimate Buyers:** 200,000 requests from actual customers.

#### Without Bangsaen Filter:
Those 800,000 malicious requests hit your origin web server directly. Your PHP-FPM worker pool saturates, MariaDB database connections lock, and your site crashes with `502 Bad Gateway` errors. To survive, you are forced to upgrade to a $150–$300/month high-spec cloud instance—**effectively paying hosting providers for bot-generated traffic.**

#### With Bangsaen Filter Pro ($19/mo):
The Edge engine obliterates all 800,000 bot requests at the door within 1.3ms of CPU time. Your origin server processes only the 200,000 legitimate requests, operating smoothly at **0.00% CPU impact from threat traffic**.

> **The Math is Simple:** Paying $19/month to destroy 800k bad bots at the Edge saves you $150–$300/month in origin hosting upgrades.
